...
const D20_FACE_CENTROIDS = d20Data.faces.map((face) => {
  const v0 = d20Data.vertices[face[0]], v1 = d20Data.vertices[face[1]], v2 = d20Data.vertices[face[2]]
...
export const D20Definition: DiceDefinition = {
  createMesh(color: DiceColor): THREE.Mesh {
    ...
    for (let t = 0; t < posAttr.count / 3; t++) {
      ...
      let bestAbsDot = -2
      for (let f = 0; f < D20_NORMALS.length; f++) {
        const absDot = Math.abs(nx * D20_NORMALS[f].x + ny * D20_NORMALS[f].y + nz * D20_NORMALS[f].z)
      ...
      if (candidates.length > 1) {
        ...
        for (const f of candidates) {
          const dot = (tcx / tlen) * D20_FACE_CENTROIDS[f].x + (tcy / tlen) * D20_FACE_CENTROIDS[f].y + (tcz / tlen) * D20_FACE_CENTROIDS[f].z
